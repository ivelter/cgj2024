...
func _ready():
	for i in PlayerInfo.maxHealthPoints:
		new_heart()

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	# find the heart that might not be full.
	var last_heart = floor(PlayerInfo.healthPoints)

	for heart in $HeartDisplay.get_children():
		...
		if index > last_heart:
			...
			heart.frame = (PlayerInfo.healthPoints - last_heart) * 4
		elif index < last_heart:
			heart.frame = 4
